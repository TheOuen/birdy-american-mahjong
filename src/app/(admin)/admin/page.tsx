...
type OrderLite = { id: string; status: string; items: { slug: string; quantity: number }[] }
type MessageLite = { id: string; status: string }
type PostLite = { id: string; published: boolean }

export default async function AdminDashboardPage() {
  const [orders, messages, posts, players] = await Promise.all([
  ...
  const newBookings = orders.rows.filter(
    (o) => o.status === 'new' && o.items?.some((i) => i.slug.includes('session'))
  ).length
  const newOrders = orders.rows.filter((o) => o.status === 'new').length
  ...
  const stats = [
    ...
    { label: 'New orders', value: newOrders, href: '/admin/orders', accent: 'var(--accent-gold)' },
